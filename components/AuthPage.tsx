...
export const AuthPage: React.FC<AuthPageProps> = ({ onClose, onAuthSuccess }) => {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    try {
        ...
        if (msg.includes("Failed to fetch")) {
            msg = "Network Error: Could not connect to Supabase. Please check your Supabase URL and ensure the project is active. Also, verify that your API key is a valid Supabase Anon Key (starts with 'eyJ'), not a Stripe key.";
        }
        setError(msg);
    } finally {
        setIsLoading(false);
  ...
  const handleSignUp = async (e: React.FormEvent) => {
    ...
    try {
        ...
        // 1. Create user in Auth with metadata for the trigger
        ...
        if (data.user) {
            // If email confirmation is ON, there might not be a session yet
            if (!data.session) {
            ...
            // If we have a session, the trigger has already created the profile
            onAuthSuccess({
                id: data.user.id,
                phone: phone.trim(),
                name: name.trim(),
        ...
        <div className="p-8 sm:p-12 relative overflow-y-auto">
            ...
            {viewMode === 'signup' && (
                <div className="animate-fade-in">
                    ...
                    {infoMessage ? (
                        <div className="bg-blue-50 text-blue-700 p-4 rounded-lg mt-2 mb-6 border border-blue-100 text-sm">
                            {infoMessage}
                        </div>
                    ) : (
